...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/256_3_testing_config.json',
...
#                 diffusion.optimize_parameters()
                ...
                if current_step % opt['train']['val_freq'] == 0:
                    ...
                    for _,  val_data in enumerate(val_loader):
                        if random.random() > 0.002: # randomly sample about 21 images from the val dataset, calculate val stats on these
                            continue
                        ...
                        if len(sr_img.shape) == 3:# this should only activate if the image is 3 channel, if one channel image, no need to transpose
                            ...
                            tb_logger.add_image(
                                ...
                                np.tile(np.concatenate(
                                    (fake_img, sr_img, hr_img), axis=1), (3, 1, 1)),
                                idx)
