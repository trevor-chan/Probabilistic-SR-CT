...
class Unet(nn.Module):
    def __init__(self, dim, out_dim = None, dim_mults=(1, 2, 4, 8), groups = 8):
        super().__init__()
        dims = [1, *map(lambda m: dim * m, dim_mults)]#channel dim #-------------------------------------------------------------------------
...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, image_size, batch_size = 16, noise=None, mask=None):
        x = self.p_sample_loop((batch_size, 1, image_size, image_size), noise, mask)#-------------------------------------------------------------------------
        return x
    ...
    def p_losses(self, x_start, t, noise = None):
        ...
        if self.loss_type == 'l1':
            ...
            loss = (noise - x_recon).abs().mean() + F.mse_loss(noise, x_recon)
        else:
...
class SimpleDataset(data.Dataset):
    def __init__(self, folder, image_size):
        super().__init__()
        self.image_size = image_size
        print('Warning: Initializing simple dataset!')

    def __len__(self):
        return 100000

    def __getitem__(self, index):
        return torch.rand(3).repeat(self.image_size, self.image_size, 1).permute(2, 0, 1)
